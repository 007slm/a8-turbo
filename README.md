...
*   **A8 DB Proxy (`a8-dbproxy`)**: 原称 `ojp-server`。核心 gRPC 中间件，负责拦截 JDBC 请求，处理 "Smart Caching" (智能缓存)，并管理数据同步。
*   **A8 UI (`a8-ui`)**: 原称 `ojp-ui`。基于 Web 的管理门户 (React/Vite)。
*   **A8 SQL Translator (`a8-sql-translator`)**: 一个基于 Python 的 Sidecar 服务 (使用 SQLGlot)，用于将 Oracle SQL 转换为 StarRocks SQL 以实现透明缓存。
...
*   **`docker-compose-sidecar.yml`**: `a8-sql-translator` Sidecar 的共享定义。
...
*   **SQL Translator**: 作为容器 `a8-sql-translator` 运行，暴露在 `a8` 网络中。
...
| **SQL Translator**| N/A | `a8-sql-translator` | 内部 Sidecar |

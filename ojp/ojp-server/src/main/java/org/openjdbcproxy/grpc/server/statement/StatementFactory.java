...
public class StatementFactory {
    ...
    public static PreparedStatement createPreparedStatement(SessionManager sessionManager,
        ...
        // [FIX] Check for translated SQL in context
        String effectiveSql = sql;
        if (currentContext != null) {
            String translatedSql = (String) currentContext.getAttribute("translated.sql");
            if (StringUtils.isNotEmpty(translatedSql)) {
                effectiveSql = translatedSql;
                // Log only if needed, avoiding excessive logs in hot path if debug disabled
            }
        }

        // 将createBasePreparedStatement的逻辑直接放在这里，避免方法抽取
        if (properties.isEmpty()) {
            ps = conn.prepareStatement(effectiveSql);
        }
        if (properties.size() == 1) {
            ...
            if (columnIndexes != null) {
                ps = conn.prepareStatement(effectiveSql, columnIndexes);
            } else if (columnNames != null) {
                ps = conn.prepareStatement(effectiveSql, columnNames);
            } else if (isAddBatch != null && isAddBatch) {
                ps = conn.prepareStatement(effectiveSql);
            } else if (autoGeneratedKeys != null) {
                ps = conn.prepareStatement(effectiveSql, Statement.RETURN_GENERATED_KEYS);
            }
        }
        ...
        if (resultSetType != null && resultSetConcurrency != null && resultSetHoldability == null) {
            ps = conn.prepareStatement(effectiveSql, resultSetType, resultSetConcurrency);
        }
        if (resultSetType != null && resultSetConcurrency != null && resultSetHoldability != null) {
            ps = conn.prepareStatement(effectiveSql, resultSetType, resultSetConcurrency, resultSetHoldability);
        }
        if (ps == null) {

...
public class TableRuleController {
    ...
    public List<CacheRule> getTableRules(
        ...
        List<CacheRule> rules = tableRuleService.getTableRules(tableName, datasource);
    ...
    public CacheRule createTableRule(
            @Parameter(description = "表名") @PathVariable String tableName,
            @Valid @RequestBody CreateTableRuleRequest request) {
        log.info("为表格创建缓存规则，表名: {}, 请求: {}", tableName, request);
        CacheRule rule = tableRuleService.createTableRule(tableName, request);
        log.info("成功为表格创建缓存规则，表名: {}, 规则ID: {}", tableName, rule.getId());
        return rule;
    }
}
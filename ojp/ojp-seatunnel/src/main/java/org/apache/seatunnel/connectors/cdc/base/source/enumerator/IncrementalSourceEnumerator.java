    ...
    public void handleSourceEvent(int subtaskId, SourceEvent sourceEvent) {
        if (sourceEvent instanceof CompletedSnapshotSplitsReportEvent) {
            ...
            if (splitAssigner instanceof HybridSplitAssigner) {
                ...
                // 注入代码：向 OJP Server 异步安全推送全量同步结束的精确事件
                try {
                    com.ojp.patch.reporter.OjpEventReporter.reportEventAsync(
                        "SNAPSHOT_FINISHED", "tables", event.getTableIds().toString()
                    );
                } catch (Throwable t) {
                    LOG.error("Failed to trigger OjpEventReporter for SNAPSHOT_FINISHED", t);
                }
            }
        }
    }

 ...
 * 
 * 字段说明:
 * queryId: 查询ID（通过SQL生成的唯一标识）
 * sql: 原始SQL语句
 * normalizedSql: 规范化的SQL语句（去除参数）
 * tables: 查询涉及的表名列表
 * parameters: SQL参数
 * parameterHash: 参数哈希值（用于缓存键生成）
...
public class Query {
    ...
    public String getCacheKey() {
        if (parameterHash != null && !parameterHash.isEmpty()) {
            return String.format("%s:cache:%s:%s", connHash, queryId, parameterHash);
        } else {
            return String.format("%s:cache:%s", connHash, queryId);
        }
    }
